...
public final class CallerOverlayManager {
    ...
    private static void showOnMain(Context context, Customer customer, String memo, String stageColor) {
        ...
        try {
            ...
            hideRunnable = CallerOverlayManager::hideOnMain;
    ...
    private static View buildView(Context context, Customer customer, String memo, String stageColor) {
        ...
        outer.setPadding(dp(context, 14), 0, dp(context, 14), 0);

        LinearLayout card = new LinearLayout(context);
        card.setOrientation(LinearLayout.VERTICAL);
        card.setPadding(dp(context, 17), dp(context, 14), dp(context, 14), dp(context, 14));
        card.setElevation(dp(context, 14));
        ...
        TextView label = text(context, "콜태그 · 등록 고객", 12f, context.getColor(R.color.text_secondary), true);
        header.addView(label, new LinearLayout.LayoutParams(0,
                LinearLayout.LayoutParams.WRAP_CONTENT, 1f));

        TextView close = text(context, "×", 22f, context.getColor(R.color.text_primary), false);
        close.setGravity(Gravity.CENTER);
        close.setBackgroundResource(R.drawable.bg_secondary_button);
        close.setOnClickListener(v -> hide(context));
        header.addView(close, new LinearLayout.LayoutParams(dp(context, 38), dp(context, 38)));
        card.addView(header);

        TextView name = text(context, customer.displayName, 21f,
                context.getColor(R.color.text_primary), true);
        LinearLayout.LayoutParams nameParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT);
        nameParams.topMargin = dp(context, 8);
        ...
        TextView stage = text(context,
                ...
                12f, contrast(parseColor(context, stageColor)), true);
        stage.setPadding(dp(context, 10), dp(context, 5), dp(context, 10), dp(context, 5));
        stage.setBackground(pill(parseColor(context, stageColor)));
        meta.addView(stage);

        TextView phone = text(context, customer.primaryPhone, 13f,
                context.getColor(R.color.text_secondary), false);
        LinearLayout.LayoutParams phoneParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.WRAP_CONTENT,
                LinearLayout.LayoutParams.WRAP_CONTENT);
        phoneParams.leftMargin = dp(context, 10);
        meta.addView(phone, phoneParams);
        LinearLayout.LayoutParams metaParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT);
        metaParams.topMargin = dp(context, 8);
        card.addView(meta, metaParams);

        String safeMemo = memo == null ? "" : memo.trim();
        TextView memoView = text(context,
                safeMemo.isEmpty() ? "저장된 최근 메모가 없습니다." : safeMemo,
                14f,
                safeMemo.isEmpty() ? context.getColor(R.color.text_muted)
                        : context.getColor(R.color.text_primary),
                false);
        memoView.setMaxLines(3);
        memoView.setLineSpacing(0f, 1.2f);
        memoView.setPadding(dp(context, 13), dp(context, 11), dp(context, 13), dp(context, 11));
        ...
        LinearLayout.LayoutParams memoParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT);
        memoParams.topMargin = dp(context, 11);
        ...
        LinearLayout.LayoutParams lastParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT);
        lastParams.topMargin = dp(context, 8);
        ...
        detail.setOnClickListener(v -> {
            ...
            try {
                ...
                // The phone screen remains usable even if opening the app is blocked.
            }
        });
        LinearLayout.LayoutParams detailParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, dp(context, 44));
        detailParams.topMargin = dp(context, 11);
    ...
    private static void hideOnMain() {
        ...
        if (windowManager != null && overlayView != null) {
            try {
                ...
                // It may already have been removed by the system.
            }
        }
        overlayView = null;
        windowManager = null;
