...
public final class PageroLeadApiClient {
    ...
    public static final class Page {
        public final List<PageroLead> leads;
        public final long nextAfter;
        public final boolean hasMore;

        Page(List<PageroLead> leads, long nextAfter, boolean hasMore) {
    ...
    public static void acknowledgeImported(
        ...
        requestWithFallback("POST", "/api/call/pagero/leads/ack", body, session);
    }
    ...
    public static void acknowledgeRejected(
            ...
            String result) throws Exception {
        JSONObject body = new JSONObject()
                .put("leadIds", new JSONArray().put(leadId))
                .put("status", "REJECTED")
                .put("result", result == null ? "" : result.trim());
        requestWithFallback("POST", "/api/call/pagero/leads/ack", body, session);
    }
    ...
    private static JSONObject requestWithFallback(
        ...
        throw lastTransport == null
                ? new Exception("페이지로 문의 서버에 연결하지 못했습니다.")
                : lastTransport;
    }
    ...
    private static JSONObject request(
        ...
        try {
            ...
            if (status < 200 || status >= 300 || !response.optBoolean("ok", true)) {
                ...
                throw new ApiException(
                        response.optString("error", "페이지로 문의를 동기화하지 못했습니다."),
                        status,
                        code);
            }
            return response;
