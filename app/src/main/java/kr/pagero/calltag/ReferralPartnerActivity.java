...
/** 더보기 > 친구 초대. 추천인 입력은 회원가입 화면에서만 허용한다. */
public final class ReferralPartnerActivity extends Activity {
        ...
        new Thread(() -> {
            ...
            runOnUiThread(() -> {
                ...
                if (notify) {
                    Toast.makeText(
                            this,
                            loaded ? "추천인 코드를 확인했습니다." : "추천인 코드를 확인하지 못했습니다.",
    ...
    private void shareCode() {
        ...
        StringBuilder message = new StringBuilder()
                .append("콜태그 가입할 때 추천인 코드를 입력하면 통합권 무료체험이 7일 추가되어 총 14일 이용할 수 있어요.\n")
                .append("추천인 코드: ")
                .append(value.code);

        if (!value.shareUrl.isEmpty()) {
