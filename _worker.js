...
export default {
  async fetch(request,env,context){
    ...
    if(isLanding){
      ...
      if(!body.includes('calltag-app-only-v1.js')){
        body=body.replace('</body>',`<script src="${LANDING_APP_ONLY}"></script></body>`);
      }
      if(!body.includes('calltag-hero-restore-v1.js')){
        body=body.replace('</body>',`<script src="${LANDING_HERO_RESTORE}"></script></body>`);
      }
    }
