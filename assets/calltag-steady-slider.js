(()=>{
  ...
  const run=()=>{
    ...
    const tick=time=>{
      ...
      last=time;
      if(!paused&&visible&&!reduced.matches&&loopWidth>0){
    ...
    viewport.addEventListener('focusout',e=>{if(!viewport.contains(e.relatedTarget)){paused=false;last=performance.now();}});
    new ResizeObserver(()=>requestAnimationFrame(measure)).observe(viewport);
    new IntersectionObserver(e=>{visible=e[0]?.isIntersecting??true;last=performance.now();},{threshold:.01}).observe(viewport);
    measure();
    frame=requestAnimationFrame(tick);
    ...
    style.textContent=`
      #targets .ct-marquee-rail{animation:none!important;transition:none!important;will-change:transform}
      #targets .ct-marquee-viewport{overflow:hidden!important}
