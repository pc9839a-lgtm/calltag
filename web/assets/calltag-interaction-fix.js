(()=>{
  if(document.documentElement.dataset.ctInteractionFixV4)return;
  document.documentElement.dataset.ctInteractionFixV4='1';
  ...
  const run=()=>{
    ...
    if(stage){
      ...
      [...items,...screens].forEach(node=>observer.observe(node,{attributes:true,attributeFilter:['class']}));activate(Math.min(Math.max(items.findIndex(item=>item.classList.contains('active')),0),visibleCount-1));window.addEventListener('pagehide',()=>observer.disconnect(),{once:true});
    }
    ...
    if(!q('style[data-ct-interaction-fix]')){
      const style=document.createElement('style');style.dataset.ctInteractionFix='4';
      style.textContent=`
