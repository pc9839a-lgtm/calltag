(()=>{
  if(document.documentElement.dataset.ctSiteFinalCleanupV5)return;
  document.documentElement.dataset.ctSiteFinalCleanupV5='1';
  ...
  const installStyle=()=>{
    if(document.querySelector('style[data-ct-site-final-cleanup-v5]'))return;
    const style=document.createElement('style');
    style.dataset.ctSiteFinalCleanupV5='1';
    style.textContent=`
  ...
  const normalizeHistory=()=>{
    ...
    const history=document.querySelector('#history');if(!history)return;
    history.querySelectorAll('.product-panel,.customer-detail,.customer-side,.customer-history,.timeline,.timeline-row,.timeline-content').forEach(node=>{node.style.removeProperty('height');node.style.removeProperty('max-height');node.style.removeProperty('overflow');});
  };
  ...
  const apply=()=>{
    ...
    normalizeHistory();
    let footer=document.querySelector('footer.footer,footer');if(!footer){footer=document.createElement('footer');document.body.append(footer);}footer.className='footer ct-wayzi-footer';if(footer.dataset.ctWayziFooter!=='5'){footer.innerHTML=footerMarkup;footer.dataset.ctWayziFooter='5';}footer.style.pointerEvents='auto';footer.style.position='relative';footer.style.zIndex='80';
  ...
  const boot=()=>{apply();const observer=new MutationObserver(queue);observer.observe(document.documentElement,{childList:true,subtree:true});const onResize=queue;addEventListener('resize',onResize,{passive:true});mobile.addEventListener?.('change',queue);const timers=[400,1200,3000].map(delay=>setTimeout(apply,delay));const stopTimer=setTimeout(()=>{apply();observer.disconnect();},5000);window.addEventListener('pagehide',()=>{timers.forEach(clearTimeout);clearTimeout(stopTimer);observer.disconnect();removeEventListener('resize',onResize);mobile.removeEventListener?.('change',queue);document.removeEventListener('click',forceNavigate,true);},{once:true});};
