...
> 이 문서는 현재 구현 상태의 정본이다. 오래된 `0.3x~0.41.x`, Classic/Pro, 기본 3일 무료체험, 추천 +5일, 통합 2,500원, 큰 통화 종료창, 시스템 CallLog 메모 쓰기, 연락처 이름 변조 방식은 현재 기준이 아니다.
...
- `HomeTaskEditorActivity`의 시간 선택은 code84부터 시스템 AM/PM TimePicker 대신 **콜태그 전용 오전/오후 + 시 + 분 칩 UI** 사용

잔여점:

- `MainActivity`의 캘린더 일정 추가/일정 변경 경로에는 아직 `TimePickerDialog`가 남아 있다.
- 다음 UI 정리에서 `TaskTimeChoiceDialog` 계열로 통일 필요.
...
code84에서 별도 밝은 화면 느낌을 제거하고 콜태그 다크 카드 톤으로 통일했다.

- 배경 다크
- 카드/버튼 콜태그 스타일
- 7일 + 추천 7일 = 총 14일 안내 문구로 수정
- 실제 Play 결제 기능은 아직 P0 미완료

### 데이터 보호·복구

code84에서 다크 UI로 통일했다.

- CallTag 헤더/카드/버튼 스타일 사용
- 기본 흰색 버튼 노출 제거
- 동기화/복구 상태를 같은 디자인 언어로 표시

## 6. 그룹·단체문자 / 스마트그룹 — code84

### 그룹·단체문자

- 허브 카드/여백/버튼을 콜태그 다크 UI에 맞춤
- 수동그룹 편집 Dialog도 CallTag Dialog 스타일 사용
- 스마트그룹 Spinner는 시스템 기본 흰 드롭다운 대신 `CallTagSpinnerAdapter` 사용
...
- 기본 Android 흰색 Dialog/Picker를 가능한 한 그대로 노출하지 않음
- 확인/등록/저장: 파란 primary
- 취소/닫기: 다크 secondary
...
- 빌드용 PR: `#68` — artifact only, **제품에 병합 금지**
- workflow run: `31233511666`
- Play AAB artifact ID: `9014694531`
- Debug APK artifact ID: `9014646024`

검증:

- code84 정적 contract 검사 PASS
- Debug APK 빌드 PASS
...
code84 Debug APK SHA-256:

`88380f064667e64ee12ebd3dcefc7ee404eda46bc02241215a70561a42eb5234`
...
- 캘린더 일정 추가/변경의 시스템 `TimePickerDialog` 제거
